...
class FlashcardApp:
    def __init__(self, master):
        self.master = master
        self.master.title("Kuis Anime")
        self.master.geometry("500x650")
        self.master.configure(bg='white')

        # Minimalist flashcard set with hints
        self.flashcards = [
            Flashcard("Siapa karakter utama di anime Naruto?", "Naruto Uzumaki"),
            Flashcard("Apa kekuatan khas Goku di Dragon Ball?", "Super Saiyan"),
            Flashcard("Di anime One Piece, apa tujuan Luffy?", "Menjadi Raja Bajak Laut"),
            Flashcard("Apa nama pedang yang digunakan oleh Kirito di Sword Art Online?", "Elucidator"),
            Flashcard("Di Attack on Titan, siapa pemimpin Survey Corps?", "Levi ackerman"),
            Flashcard("Siapa karakter dewa kematian di Death Note?", "Ryuk"),
            Flashcard("Siapa karakter utama di anime One Punch Man?", "Saitama"),
            Flashcard("Di Naruto, siapa Hokage keempat?", "Minato Namikaze"),
            Flashcard("Siapa karakter utama di Sword Art Online?", "Kirito"),
            Flashcard("Apa identitas asli Heathcliff di anime Sword Art Online?", "Kayaba Akihiko"),
            Flashcard("Siapa karakter utama di Demon Slayer?", "Kamado Tanjiro"),
            Flashcard("Siapa teman dekat Naruto yang menjadi rival sekaligus sahabat?", "Sasuke Uchiha"),
            Flashcard("Siapa sniper wanita di Sword Art Online di GGO?", "Sinon"),
            Flashcard("Siapa karakter utama Black Clover yang bercita-cita menjadi Wizard King?", "Asta"),
            Flashcard("Siapa karakter utama di Noragami yang merupakan dewa kemiskinan?", "Yato")
        ]

        # Hints corresponding to each flashcard (in the same order)
        self.hints = [
            "Ninja dengan impian jadi Hokage",
            "Transformasi dengan rambut pirang",
            "Topi jerami adalah ciri khasnya",
            "Pedang hitam yang sering dipakai",
            "Kapten ahli membasmi Titan",
            "Suka apel",
            "Botak",
            "Si cepat kilat",
            "Nama aslinya Kirigaya Kazuto",
            "Pencipta game Sao",
            "Memakai hanafuda earrings dan pedang Nichirin",
            "Anggota klan Uchiha, ahli Sharingan",
            "Pemain dengan senjata jarak jauh, trauma masa lalu",
            "Tidak memiliki sihir tapi kuat dengan pedang anti-sihir",
            "Dewa dengan kostum biru dan target 5 yen"
        ]
